...
internal class NetworkConnectivityReceiverTest {
    ...
    fun emitIfChanged_forwardsChangedStateOnly() = runTest {
        val connectedStates = ArrayDeque(listOf(false, true, true))
        val scope = TestScope(StandardTestDispatcher(testScheduler))
        val receiver = NetworkConnectivityReceiver(service, scope, logger, null) {
        ...
        coVerify(exactly = 1) { service.handleEvent(NetworkConnectivityEvent(true)) }
    }
}
