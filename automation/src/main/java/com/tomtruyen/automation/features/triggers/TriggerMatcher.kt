...
class TriggerMatcher(delegates: List<TriggerDelegate<out TriggerConfig>> = GeneratedTriggerProvider.delegates()) {
    private val delegatesByType = delegates.associateBy { it.type }

    fun matches(triggers: List<TriggerConfig>, event: AutomationEvent): Boolean = triggers.any { trigger ->
        delegatesByType[trigger.type]?.matchesTyped(trigger, event) ?: false
    }
}

@Suppress("UNCHECKED_CAST")
private fun TriggerDelegate<out TriggerConfig>.matchesTyped(config: TriggerConfig, event: AutomationEvent): Boolean =
    (this as TriggerDelegate<TriggerConfig>).matches(config, event)
