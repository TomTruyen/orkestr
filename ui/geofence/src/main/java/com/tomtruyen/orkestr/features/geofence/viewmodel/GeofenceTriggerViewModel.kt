    ...
    private fun saveConfiguration() {
        ...
        if (config.geofenceId.isBlank()) {
            updateState { state ->
                state.copy(
                    configErrors = listOf(
                        stringResolver.resolve(AutomationR.string.automation_definition_trigger_geofence_error_missing_geofence),
                    ),
                )
            }
            return
        ...
        updateState { state ->
            state.copy(
                geofenceEditorState = GeofenceEditorState(
                    ...
                    latitudeText = formatCoordinate(sourceGeofence?.latitude ?: 0.0),
                    longitudeText = formatCoordinate(sourceGeofence?.longitude ?: 0.0),
                    radiusText = formatRadius(sourceGeofence?.radiusMeters ?: 150f),
                    mapLatitude = sourceGeofence?.latitude ?: 0.0,
                    mapLongitude = sourceGeofence?.longitude ?: 0.0,
                    mapRadiusMeters = sourceGeofence?.radiusMeters ?: 150f,
                ),
            )
        }
